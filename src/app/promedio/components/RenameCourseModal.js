...
export default function RenameCourseModal({ open, value, onChange, onCancel, onConfirm }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div role="dialog" aria-modal="true" className="relative card w-[min(92vw,480px)] p-5 sm:p-6">
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="button" className="btn btn-primary" onClick={onConfirm}>Guardar</button>
        </div>
      </div>
    </div>
  );
